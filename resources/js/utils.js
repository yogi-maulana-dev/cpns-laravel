...
export const isCheckboxCheckedAll = (className) => {
    return Boolean(
        [...document.querySelectorAll(className)].filter(
            (checkbox) => checkbox.checked
        ).length
    );
};

export const getElement = (selector, cbIfExist) => {
    return document.querySelector(selector)
        ? cbIfExist(document.querySelector(selector))
        : false;
};
